...
The CLI is called **`yaad`** (registered as a console script by
`pyproject.toml`). There are three ways to run the app:

- **Docker Compose** (recommended for a persistent deployment): two
  containers — the FastAPI backend and a Caddy-served Svelte frontend —
  brought up with one command, config/state persisted on a bind-mounted
  volume.
- **The web app run directly** (`yaad serve`): same backend+frontend,
  no containers — better for local development.
- **The bare CLI + cron** — the original design, still fully supported
  for anyone who'd rather not run a server at all.

## Docker Compose

1. **Create the persisted data directory** (holds `config.yaml` and
   `state/` across container rebuilds):
   ```sh
   mkdir -p data
   ```
   If you already have a `config.yaml` from a previous non-Docker setup,
   copy it in: `cp config.yaml data/config.yaml && chmod 600 data/config.yaml`.
   Otherwise the onboarding wizard creates it for you on first visit.

2. **Build and start both containers:**
   ```sh
   docker compose up -d --build
   ```
   Open **http://localhost:8180** (the frontend/Caddy port) — that's the
   app. The backend is also reachable directly at **http://localhost:8100**
   if you want to hit the API without going through the frontend.

   First run walks you through onboarding exactly as described below.
   The daily schedule fires from inside the `backend` container as long
   as it's running — `docker compose ps` / `restart: unless-stopped`
   keep it up across reboots and crashes.

3. **Logs:**
   ```sh
   docker compose logs -f backend    # scheduler, fetch/sync outcomes, API access log
   docker compose logs -f frontend   # Caddy access log
   ```

4. **Ports** (`8100`, `8180`) are just what happened to be free when this
   was set up — change the host side of the `ports:` mappings in
   `docker-compose.yml` if either collides with something else on your
   machine.

Architecture note: the frontend container is Caddy serving the built
Svelte static files, reverse-proxying `/api/*` to the backend container
by its Compose service name (`frontend/Caddyfile`) — mirroring how the
Vite dev-server proxy works for local development, so the browser only
...
1. **Backend — create a venv and install:**
   ...
   ```
   This registers the `yaad` command inside the venv
   (`.venv/bin/yaad`) — activate the venv (`source .venv/bin/activate`)
...
3. **Run the server:**
   ...
   From there you land on the Home screen: today's times, a toggle +
   minutes-before stepper per prayer (autosaves), and a sync-status card.
   ...
   deliberately (Docker Compose does this for you, binding `0.0.0.0`
   inside the container and mapping a host port instead).
...
4. **Daily sync**: as long as `yaad serve` is running, it fetches +
   syncs itself once a day at the time set in the Settings page's
   Schedule card (default 03:00, editable without restarting). Keep the
   server running for this to actually happen unattended — see "Running
   persistently" below.

## Using the app

Once setup is done, opening the app takes you to the **Home** screen:
...
- **Prayers card**: each row shows today's actual Adhan/Iqama time for
  ...
  many minutes before Iqama the alarm should fire (type directly into it,
  or use −/+), and a toggle to enable/disable that prayer entirely. Both
  **autosave** a moment after you change them — watch for the small
  "Saving…" / "Saved" label. Disabling a prayer here doesn't just stop
  future syncing of it — the next sync actively **removes** its event
  from the calendar if one was already there.
- **Sync card**: shows when the daily job last ran and whether it
  succeeded (with the error message if not). **Sync Now** triggers a
  fetch + sync immediately, outside the daily schedule.
- **The gear icon** (top-right of Home) opens the **Settings** page:
  - **Appearance**: a Light mode toggle. Defaults to your system's
    light/dark preference on first visit, then remembers whatever you
    pick — stored per-browser, not in `config.yaml` (it's a display
    preference, not a server setting).
  - **Mosque / iCloud / Schedule cards**: each needs an explicit **Save**
    (unlike the Prayers card) because saving re-validates against
    Mawaqit or iCloud first. On the iCloud card, leave the password
    field blank to keep the one already on file — only fill it in when
    you're actually rotating it.
  - The back-chevron returns to Home.

### Running persistently

If you're not using Docker Compose (which already has `restart:
unless-stopped`), `yaad serve` needs to keep running for the daily
schedule to fire. A simple systemd user service:
...
Skip `npm`/`serve`/Docker entirely and drive it by hand or via cron instead:
...
personal UI is fastest to verify by actually looking at it);
